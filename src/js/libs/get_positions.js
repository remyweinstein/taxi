...
define(['Ajax', 'Car', 'Maps', 'Dates', 'Dom', 'Geo'], function (Ajax, Car, Maps, Dates, Dom, Geo) {
  ...
  function get_pos_one_client(type) {
    if (type === 'order') {
      Model = MyOrder;
    } else {
      Model = MyOffer;
  ...
  function get_pos_one_driver(type) {
    ...
    Ajax.request('GET', 'bid', User.token, '&id=' + Model.bid_id, '', function (response) {
      if (response && response.ok) {
        ...
        var toLoc = ords.toLocation.split(','),
        ...
        if (!MapElements.markers[0]) {
          var VLatLng = new google.maps.LatLng(loc[0], loc[1]);

          MapElements.markers[0] = new google.maps.Marker({
            position: VLatLng,
            map: map,
            icon: driver_icon,
            title: 'Водитель'
          ...
          MapElements.markers[0].setPosition(new google.maps.LatLng(loc[0], loc[1]));
        }
      }
