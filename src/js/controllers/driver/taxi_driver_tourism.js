...
define(['Lists', 'Storage', 'ModalWindows', 'DriverOffer', 'Dom'], 
function (Lists, Storage, Modal, clDriverOffer, Dom) {
  ...
  function cbGetOrders(response) {
    ...
    if (!response.error) {
      Lists.allOrdersTourism(response.result);
    }
    ...
    function stopStartOrders() {
      ...
      Conn.request('startGetTourismOrders', '', cbGetOrders);
    }
  }
  ...
  function start() {
    ...
    Lists.filtersStart();
    Conn.request('startGetTourismOrders', '', cbGetOrders);
    Conn.request('requestMyTourismOffers', '', cbMyOffers);
