/* global User, google, map, SafeWin, driver_icon, Event, MapElements, MyOffer */

define(['Ajax', 'Dom', 'Chat', 'Dates', 'Geo', 'Maps', 'HideForms', 'GetPositions', 'Destinations'],
  function (Ajax, Dom, Chat, Dates, Geo, Maps, HideForms, GetPositions, Destinations) {
  
  var order_id, timerStatusOffer, fromAddress, toAddress, fromCoords, toCoords, waypoints, price, name_client, photo_client;
    ...
    Event.click = function (event) {
      ...
      while (target !== this) {
        ...
        if (target.dataset.click === "driver-arrived") {
          Ajax.request('POST', 'arrived-bid', User.token, '&id=' + bid_id, '', function() {}, Ajax.error);
        }
  ...
  function start() {
    ...
    timerStatusOffer = setInterval(get_status_offer, 3000);
